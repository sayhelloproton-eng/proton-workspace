...
最早的工程链路很笨重：本机仓库人工打包给 Web Chat，Chat 再生成 planner / executor handoff，本地执行器修改后重新打包审计。08-01～08-02 的旧平台阶段记录保留了这段演进；原仓已经退休，因此这里保留事件和因果链，不再把那些已不可回读的旧 SHA 当成当前 Evidence Anchor。
