...
这条学习路径也不按固定周数或“看完多少章节”推进，而按**能力 Gate（能力门：只有前一层已经能被真实验证，才进入下一层复杂度）**推进。一个新概念至少要能回到真实项目完成最小实验，留下证据、失败和适用边界，并说明下一层为什么依赖它；否则“学过”只代表接触过名词，不代表已经形成可复用工程能力。产品菜单、配置键和 UI 会快速变化，实施前重新核验；真正长期保留的是责任边界、验证方法和经过实践反复成立的工程判断。
