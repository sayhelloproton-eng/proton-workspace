...
这篇文章不讨论哪家 Agent 框架更强，也不再重复 [Coding Agent](../工程工具与运行时/Coding-Agent.md) 里已经展开的 Model、Harness、Runtime 和 Workspace 生态差异；也不重复 [本机 Agent 工具链](../工程工具与运行时/本机Agent工具链.md) 里 Chat 怎样获得本机执行能力。这里只回答一个更基础的问题：
...
这也是为什么 [Coding Agent](../工程工具与运行时/Coding-Agent.md) 里会把 Harness 和 Runtime 放在模型之外重点讨论。任务一旦进入真实世界，模型能力只是其中一层。
